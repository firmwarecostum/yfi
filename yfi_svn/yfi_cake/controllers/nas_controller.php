...
class NasController extends AppController {
    ...
    function beforeFilter() {
       ...
       $this->Auth->allow('json_nas_map_public','json_nas_map_info_public');       //Comment out to remove public display of Google Map overlay
    ...
    */
    ...
    function json_nas_map_public($type = null){   //Public view for NAS devices
        $this->layout = 'ajax';
        ...
        //Get Sane defaults
        //Note: WE are not using this currently, though its possible!!!
        $json_return['maps']['lat']     = -25.74470879185722;
        $json_return['maps']['lon']     =  28.27759087085724;
        $json_return['maps']['type']    = 'G_SATELLITE_MAP';
      ...
      //  $r = $this->Na->find('all',array('conditions' => array('Na.lat <>' => null, 'Na.lon <>' => null))); //Get all the NAS devices without lon and lat defined
        $r = $this->Na->find('all',array('conditions' => array())); //Get all the NAS devices without lon and lat defined
        ...
        foreach($r as $entry){
            ...
            if($entry['Na']['monitor'] == 1){
             ...
             if($type == 'full'){
                array_push($json_return['maps']['items'],array(
                    'id'            =>  $na_id,
                    'name'          =>  $entry['Na']['shortname'],
                    'lon'           =>  $entry['Na']['lon'],
                    'lat'           =>  $entry['Na']['lat'],
                    'available'     =>  $avail,
                    'description'   => $entry['Na']['description'],
                    //Realm attached detail
                    'r_name'        => $entry['Realm']['name'],
                    'r_icon'        => $entry['Realm']['icon_file_name'],
                    'r_phone'       => $entry['Realm']['phone'],
                    'r_fax'         => $entry['Realm']['fax'],
                    'r_cell'        => $entry['Realm']['cell'],
                    'r_mail'        => $entry['Realm']['email'],
                    'r_url'         => $entry['Realm']['url'],
                    'r_lat'         => $entry['Realm']['lat'],
                    'r_lon'         => $entry['Realm']['lon'],
                    'r_street_no'   => $entry['Realm']['street_no'],
                    'r_street'      => $entry['Realm']['street'],
                    'r_town'        => $entry['Realm']['town_suburb'],
                    'r_city'        => $entry['Realm']['city'],
                ));
            }else{
                array_push($json_return['maps']['items'],array(
                    'id'            =>  $na_id,
    ...
    function json_nas_map_info_public($id){
        ...
        $r = $this->Na->findById($id); 

        if($r['Na']['monitor'] == 1){
                    //Get the last state
            if(count($r['NaState']) > 0){
                $last_state = $r['NaState'][0]['state'];
                if($last_state == '1'){
                    $avail = true;
                }
                if($last_state == '0'){
                ...
                $state_time = $this->Formatter->diff_in_time($r['NaState'][0]['created']);
                $last_state = $last_state." ( $state_time )";
            }
        }

        $json_return['json']['detail']= array(
                    'id'            =>  $id,
                    'name'          =>  $r['Na']['shortname'],
                    'lon'           =>  $r['Na']['lon'],
                    'lat'           =>  $r['Na']['lat'],
                    'available'     =>  $avail,
                    'description'   => $r['Na']['description'],
                    //Realm attached detail
                    'r_name'        => $r['Realm']['name'],
                    'r_icon'        => $r['Realm']['icon_file_name'],
                    'r_phone'       => $r['Realm']['phone'],
                    'r_fax'         => $r['Realm']['fax'],
                    'r_cell'        => $r['Realm']['cell'],
                    'r_mail'        => $r['Realm']['email'],
                    'r_url'         => $r['Realm']['url'],
                    'r_lat'         => $r['Realm']['lat'],
                    'r_lon'         => $r['Realm']['lon'],
                    'r_street_no'   => $r['Realm']['street_no'],
                    'r_street'      => $r['Realm']['street'],
                    'r_town'        => $r['Realm']['town_suburb'],
                    'r_city'        => $r['Realm']['city'],
                );
    ...
    function json_view_optional($id=null){
        ...
        //Add default realm
        $json_return['Na']['realm_id'] = $qr['Realm']['id'];
    ...
    function json_edit_optional(){
        ...
        if(!array_key_exists('status',$json_return['json'])){
            ...
            $d['Na']['lon']         = $this->params['form']['lon'];
            $d['Na']['lat']         = $this->params['form']['lat'];
            $d['Na']['realm_id']    = $this->params['form']['realm_id'];
