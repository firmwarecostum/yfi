...
sub prepare_statements {
    ...
    $self->{'statements'}   =   [ 
                                    'na_nasname', 'na_id', 'device_name','device_update_id', 'user_id', 'user_username','radcheck_username_password',
    ...
    $self->{'radcheck_username_password'}  = $self->{'db_handle'}->prepare("SELECT value FROM radcheck WHERE username=? and attribute='Cleartext-Password'");
    ...
    $self->{'disconnect_count_for_mac'} = $self->{'db_handle'}->prepare("SELECT COUNT(id) AS sum from actions WHERE na_id=? AND command=? AND status=?");
    ...
    $self->{'add_nas_action'}       = $self->{'db_handle'}->prepare("INSERT INTO actions(id,na_id,command,created,modified)VALUES(?,?,?,now(),now())");
