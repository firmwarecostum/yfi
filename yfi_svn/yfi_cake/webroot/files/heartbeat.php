...
try {
    ...
    //==================
    //== Check if there are a heartbeat entry for this NAS. If so update the modified time, if not add a new one
    //==================

    $nas_id = $result['id'];
    //Check there are a heartbeat entry for this device
    $stmt_hb_id = $dbh->prepare("SELECT id FROM heartbeats WHERE na_id= :nas_id");
    $stmt_hb_id->bindParam(':nas_id',$nas_id);
    $stmt_hb_id->execute();

    $result     = $stmt_hb_id->fetch(PDO::FETCH_ASSOC);
    if($result == ''){
        $stmt_hb_new = $dbh->prepare("INSERT INTO heartbeats(na_id,created,modified) VALUES (:nas_id,now(),now())");
        $stmt_hb_new->bindParam(':nas_id',$nas_id);
        $stmt_hb_new->execute();
    }else{
        $stmt_hb_upd = $dbh->prepare("UPDATE heartbeats SET modified=now() WHERE na_id=:nas_id");
